...
public class VentaController {
    ...
    @PostMapping("/crear")
    public ResponseEntity<?> crear(@RequestBody CrearVentaDTO dto) {
        try {
            VentaResponseDTO response = service.crearVenta(dto);
            ...
            return ResponseEntity.status(HttpStatus.CONFLICT)
                    .body(new ErrorResponse("Validación", e.getMessage()));
        } catch (Exception e) {
            return ResponseEntity.internalServerError()
                    .body(new ErrorResponse("Error", "No se pudo crear la venta"));
        }
    }
}
